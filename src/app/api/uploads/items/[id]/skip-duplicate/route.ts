...
export async function POST(
  ...
  if (item.duplicateStatus !== "DUPLICATE") {
    return Errors.validation("Item must have DUPLICATE status to be skipped");
  }
  if (item.duplicateImageId === null) {
    return Errors.validation("Item has no duplicate image reference");
  }

  await prisma.uploadItem.update({
    where: { id },
    data: { duplicateStatus: "SKIPPED" },
  });
