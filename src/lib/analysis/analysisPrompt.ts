...
- 次の大カテゴリから、テキストで明確に読み取れるものだけを選ぶ。優先順位は上から順:
  1. 時間帯（最優先）: 朝 / 昼 / 夕方 / 夜
     テキストに時間や光の描写が明示またはかなり強く示唆される場合だけ、対応する
     時間帯タグを出す。不確かな場合は時間帯タグを出さない。
     入力が英語表現のみでも対応する日本語タグを出すこと。対応例:
     ...
     注意: golden hour は朝夕どちらの光にも使われる語のため、単独では時間帯タグを出さない。
     morning golden hour / sunrise golden hour のように朝を示す語が伴えば「朝」、
     evening golden hour / sunset golden hour のように夕方を示す語が伴えば「夕方」とする。
