...
## 画像プライバシー / EXIF・GPS 方針

- **original 画像はそのまま保存される**ため、撮影メタデータ（EXIF、位置情報 GPS を含む）が
  **残っている可能性がある**。アップロード時にサーバー側で再エンコードや strip は行っていない。
- thumbnail / preview は Canvas（Quick Add）または sharp WebP 変換（XLSX import）で生成され、
  **通常メタデータは保持されない想定**（メタデータを保持する実装ではない）。※「必ず strip される」と
  断定はできない。
- 現在の運用前提は **private bucket `photobox-private` + 短時間 signed URL + 単一ユーザー**。
  この前提では EXIF/GPS 残存の実リスクは限定的。
- ただし original を**外部共有・公開する場合は EXIF/GPS 漏洩リスクがある**。
- 将来「共有 URL / 外部公開 / チーム利用」を導入する時点で、original の
  **EXIF/GPS strip + orientation 正規化**を設計・実装する（sharp は依存済み。[OPERATIONS.md](OPERATIONS.md) 参照）。
